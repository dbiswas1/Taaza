...
<?php
	...
	$select_price=mysql_query("select date_format(i_date,'%m-%d-%Y') as i_date,price from indent where indent_no='$_GET[gn_in_i_no]'");
	$select_price_arr = mysql_fetch_assoc($select_price);
	$pr=$select_price_arr['price'];
	//echo $pr;
...
<head>
<meta charset="utf-8">
<title>Taaza Tarkari- Add Indent</title>
...
<div id="main">
  <div class="container">
    ...
    <div id="main_container">
      <div class="row-fluid">
        <div class="span12">
        <form action="view_indent.php?indent=active&in_indent=in&v_indent=active" method="post">
        
        
          <div class="box paint color_25">
            <div class="title">
              <h4> <i class="icon-book"></i><span>Edit Indent </span> </h4>
              <div class="span4">
              <select name="cl_id"  class="chzn-select">
                  ...
                  <div class="span4" >
                    ...
                    <label class="inline radio">
                    	<?php if ($pr == 2){ ?>
                      <input type="radio" name="price" value="2" checked="CHECKED"/>
                      <?php } else {?>
              ...
              <table id="datatable_example" class="responsive table table-striped table-bordered" style="width:100%;margin-bottom:0; ">
                ...
                ?>
                  ...
                  ?>
                    	...
                    	<?php 
                    		$in_order_result=mysql_query("select qty from indent_order where i_indent_no='$_GET[gn_in_i_no]' and i_item_code='$item_arr[item_code]'");
                    		$in_order_arr=mysql_fetch_assoc($in_order_result);
                    		if(isset($in_order_arr['qty'])){

                    	?>
                    	<td class="to_hide_phone"> <input name="<?php echo $item_arr['item_code'] ; ?>" class="row-fluid span4" type="text" value="<?php echo $in_order_arr['qty'] ; ?>" placeholder="<?php echo $in_order_arr['qty'] ; ?>"  onclick="this.select();" onfocus="this.select();"  onblur="extractNumber(this,2,false);" onkeyup="extractNumber(this,2,false);" onkeypress="return blockNonNumbers(this, event, true, false);"> </td>
                    	<?php } else { ?>
                    	<td class="to_hide_phone"> <input name="<?php echo $item_arr['item_code'] ; ?>" class="row-fluid span4" type="text" placeholder="0" onclick="this.select();" onfocus="this.select();"   onblur="extractNumber(this,2,false);" onkeyup="extractNumber(this,2,false);" onkeypress="return blockNonNumbers(this, event, true, false);"> </td>
               ...
               <div class="accordion" id="accordion3">
                <div class="accordion-group">
                  ...
                  <?php 
                  	$text_msg=mysql_query("select notes from indent where indent_no ='$_GET[gn_in_i_no]'");
                  	$text_msg_arr=mysql_fetch_assoc($text_msg);
                  	$conn->close();
                  
                  ?>
                    <div class="accordion-inner"><textarea id="text" name="notes" rows="3" class="row-fluid"><?php echo $text_msg_arr['notes'] ;?></textarea></div>
                  </div>
                </div>
       ...
       <p align="center"> <button  type="submit" name="in_ed_btn" value="inden" class="btn btn-primary"> Save Changes</button> </p>
       <p align="center"> <button   name="c_btn" value="cancel" class="btn btn-primary" onclick="location.href='view_indent.php?indent=active&in_indent=in&v_indent=active'" > cancel</button> </p>
        </div>
        <input type="hidden" name="formid" value="<?php echo $_SESSION["formid"]; ?>" />
        <input type="hidden" name="indent_no_gen" value="<?php echo $_GET['gn_in_i_no']; ?>" />
        </form>
        <!-- End Box --> 
