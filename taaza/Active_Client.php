...
<?php
	...
	if (  isset($_POST["Editclient"])  || isset($_POST["clientkey"]) ||  isset($_POST["act"]) )
	{
	...
	if ($_POST["formid"] == $_SESSION["formid"])
	{
		...
		if (  isset($_POST["Editclient"]))
			mysql_query("update client set shop_name='$_POST[E_shop_name]',client_name='$_POST[E_client_name]', ph_num='$_POST[E_ph_no]', address='$_POST[E_ad_txt]',dues= '$_POST[E_dues]' where c_id='$_POST[cl_id]' ") ;
		if (  isset($_POST["clientkey"]))
			
			mysql_query("update client set status=0 where c_id='$_POST[clientkey]'");
		if (  isset($_POST["act"]))
			mysql_query("update client set status=1 where c_id='$_POST[act]'");
...
<head>
<meta charset="utf-8">
<title>Taaza Tarkari - View Active Clients</title>
 ...
 <?php if (isset($_GET["ac_client"])){ ?>
    ...
    <div id="main_container">
      <div class="row-fluid">
        <div class="box color_24">
          <div class="title">
            <h4> <span>Active Clients table <small>(Sorted in descending order)</small> </span> </h4>
          </div>
          <!-- End .title -->
          
          <div class="content top">
          <form id="del_form" action="Active_Client.php?client=active&ac_client=active" method="post">
            <table id="datatable_example" class="responsive table table-striped table-bordered" style="width:100%;margin-bottom:0; ">
              ...
              <tbody>
              <?php 
              	$result=mysql_query("select dues,c_id,shop_name,client_name,ph_num,address,date_format(join_date, '%D-%b-%Y') as jdate from client where status=1");
              	while ($client_arr=mysql_fetch_array($result)){
              ?>
              
                <tr>
                  <td><?php echo $client_arr['c_id'] ; ?></td>
                  <td><?php echo $client_arr['client_name'] ; ?></td>
                  <td class="to_hide_phone"><?php echo $client_arr['shop_name'] ; ?></td>
                  <td class="to_hide_phone"> <?php echo $client_arr['ph_num'] ; ?> </td>
                  <td class="to_hide_phone"><?php echo $client_arr['jdate'] ; ?></td>
                  ...
                  <td class="ms">
                  	<div class="btn-group" id="<?php echo $client_arr['c_id'] ;?>"> 
						...
						<a class="btn  btn-small" rel="tooltip" data-placement="top" data-original-title="Remove" onclick="javascript:openDialog(<?php echo $client_arr['c_id'] ;?>);"><i class="gicon-remove"></i></a> 
						<input type="hidden" name="cremove" value="<?php echo $client_arr['c_id'] ;?>" />
    ...
    <?php } if(isset($_GET["dp_client"])){ ?>
      ...
      <div class="row-fluid">
        <div class="box color_24">
          <div class="title">
            <h4> <span>In-Active Clients table <small>(Sorted in descending order)</small> </span> </h4>
          </div>
          <!-- End .title -->
          ...
          <form id="del_form" action="Active_Client.php?client=active&dp_client=active" method="post">
            <table id="datatable_example" class="responsive table table-striped table-bordered" style="width:100%;margin-bottom:0; ">
              ...
              <tbody>
              <?php 
              	$result=mysql_query("select dues,c_id,shop_name,client_name,ph_num,address,date_format(join_date, '%D-%b-%Y') as jdate from client where status=0");
              	while ($client_arr=mysql_fetch_array($result)){
              ?>
