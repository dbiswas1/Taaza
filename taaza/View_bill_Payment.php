...
<?php
	...
	if ($_POST["formid"] == $_SESSION["formid"])
	{
		...
		$cur_due_query=mysql_query("select paid from bill_payment_master where bl_id='$_POST[bl_id]'");
		$cur_diff_arr=mysql_fetch_assoc($cur_due_query);
		$curr_diff= $cur_diff_arr['paid'] - $_POST['pay'] ;
		$update_query = "update bill_payment_master set bill_b_id=".$_POST['b_id'].", paid=".$_POST['pay'].",date=STR_TO_DATE('".$_POST['date']."','%m-%d-%Y')".", pay_b_no=".$_POST['vouch'].", dues=dues+$curr_diff where bl_id=".$_POST['bl_id'];
		//echo $update_query;
		...
		mysql_query("update biller set dues=(dues+$curr_diff) where b_id=$biller_id");
...
<head>
<meta charset="utf-8">
<title>Taaza Tarkari - View Bill Payments</title>
   ...
   <!--   <?php  //if(isset($_GET["dp_client"])){ ?> -->
      ...
      <div class="row-fluid">
        <div class="box color_13">
          <div class="title">
            <h4> <span>Purchase Payments Summary table <small>(Sorted in descending order)</small> </span> </h4>
          </div>
          <!-- End .title -->
          
          <div class="content top">
          <form id="del_form" action="view_bill_payment.php?purchase=active&in_purchase=in&v_purchase=active" method="post">
            <table id="datatable_example" class="responsive table table-striped table-bordered" style="width:100%;margin-bottom:0; ">
              ...
              ?>
                ...
                <tr>
                  ...
                  <td class="to_hide_phone"><?php echo $client_arr['pay_b_no'] ; ?></td>
                  <td class="ms">
                  	<div class="btn-group" id="<?php echo $client_arr['bl_id'] ;?>"> 
                  		<a class="btn btn-small" href="Edit_bill_Payment.php?purchase=active&in_purchase=in&v_purchase=active&market_name=<?php echo $client_arr['market_name'] ; ?>&bid=<?php echo $client_arr['b_id'] ;?>&bl_id=<?php echo $client_arr['bl_id'] ;?>" rel="tooltip" data-placement="top" data-original-title=" Edit " ><i class="gicon-edit"></i></a> 		
